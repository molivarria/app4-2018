...
function onDeviceReady(){
	...
	db = window.openDatabase("contactos_fimaz", "1.0", "DB del contactos FIMAZ", 200000);
	if(existe_db == null){
		creaDB();
	}else{
		cargaDatos();
...
function creaNuevaDB(tx){
	...
	tx.executeSql('DROP TABLE IF EXISTS contactos_fimaz');
	
	var sql = "CREATE TABLE IF NOT EXISTS contactos_fimaz ( "+
	...
	tx.executeSql("INSERT INTO contactos_fimaz (id,nombre,apellidos,telefono,categoria,foto,email) VALUES (1,'Mónica','Olivarría','6699811560','docente','','m.olivarria@uas.edu.mx')");
	tx.executeSql("INSERT INTO contactos_fimaz (id,nombre,apellidos,telefono,categoria,foto,email) VALUES (2,'Rodolfo','Bustamante','6699811560','alumno','','rodolfo@bustamante.com')");	
}
...
function cargaRegistros(tx){
	...
	tx.executeSql('SELECT * FROM contactos_fimaz', [], cargaDatosSuccess, errorDB);
}
...
function queryDBFindByID(tx) {
    tx.executeSql('SELECT * FROM contactos_fimaz WHERE id='+$.id, [], queryDetalleSuccess, errorDB);
}
...
function queryDBFindByIDForm(tx) {
    tx.executeSql('SELECT * FROM contactos_fimaz WHERE id='+$.id, [], queryFormSuccess, errorDB);
}
...
function initForm(){
	...
	$("#cat_docentes").trigger("click").trigger("click")
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE contactos_fimaz SET nombre="'+$("#ti_nombre").val()+'", apellidos="'+$("#ti_apellidos").val()+'",telefono="'+$("#ti_telefono").val()+'",email="'+$("#ti_mail").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO contactos_fimaz (nombre,apellidos,telefono,categoria,foto,email) VALUES ('"+$("#ti_nombre").val()+"','"+$("#ti_apellidos").val()+"','"+$("#ti_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#ti_mail").val()+"')", [], newFormSuccess, errorDB);
}
